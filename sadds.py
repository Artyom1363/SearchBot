...
def request(settings, cursor, connection, ans_id=-1, sentence=''):
    ...
    if ans_id == -1 and sentence == '':
        ...
        # make fl
        fl = insert_by_answer_id(ans_id, connection, cursor, settings)
    ...
    ustate.set_state(USER_ID_TELEG, 'adding_answer', cursor,
                     connection)

    bot.send_message(USER_ID_TELEG,
                     mes_to_user,
                     reply_markup=get_markup_cancel(),
                     parse_mode='Markdown')


def insert_by_answer_id(ans_id, connection, cursor, settings):
    sen_id = handler_sentences.get_sentence_id_by_ans_id(ans_id,
                                                         cursor, connection)

    return handler_sentences.insert_sentence_to_user(config.firstPart + config.toSplit + str(sen_id),
                                                     settings[0],
                                                     connection,
                                                     cursor)
